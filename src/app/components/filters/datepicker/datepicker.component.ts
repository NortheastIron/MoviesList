...
type TCalendarEventParams = {
  type: CalendarEventTypes;
  value?: Date;
};

type THeaderElement = {
  type: CalendarNavigateTypes;
  value: string;
};
  ...
  private _initCalendar(element: ElementRef): void {
    ...
    this._calendarSubscription = this._datepickerCalendar.subscribe({
      next: (params: TCalendarEventParams) => {
        if (params.type === CalendarEventTypes.selected && params.value) {
          this._configSelectedDate(params.value);
        } else if (params.type === CalendarEventTypes.close) {
...
class DatepickerCalendar extends EventEmitter {
  ...
  private _returnSelectedDate(date: Date): void {
    this.emit({type: CalendarEventTypes.selected, value: date});
  }

  private _closeCalendar(): void {
    this.emit({type: CalendarEventTypes.close});
  }
}
