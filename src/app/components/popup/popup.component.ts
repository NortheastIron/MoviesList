...
export class PopupComponent implements OnDestroy {
  ...
  private _initPopupPosition(): void {
    ...
    const position: PopupPosition = this._getPosition(popupRect.height, space);
    const baseAvailableSpace: number = position === PopupPosition.bottom ? space.bottom : space.top;
    const custPopupHeight: number = this._getAvailablePopupHeight(baseAvailableSpace);
    
    const top: number = position === PopupPosition.bottom 
    ? hostRect.bottom + 5
  ...
  private _getPosition(popupHeight: number, space: TSpace): PopupPosition {
    const canFitBottom = space.bottom > popupHeight;
    const hasMoreSpaceBelow = space.bottom > space.top;

    return canFitBottom || hasMoreSpaceBelow ? PopupPosition.bottom : PopupPosition.top;
  }
